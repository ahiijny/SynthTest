...
WaveWriter ww("Audio/CODE06_Soft.wav", sampleRate);
...
void writeSoftState()
{
    state = (char)((initialState - targetState) * exp(-0.04 * softFactor * timeSinceLastChange) + targetState);
   // cout << (initialState - targetState) * exp(-0.04 * softFactor * timeSinceLastChange) << " ";
...
void digitalWrite(int pin, int value)
{
    ...
    initialState = (int)state & 0xFF;
    targetState = (int)value & 0xFF;
}
...
void delayMicroseconds (int duration)
{
	...
	{
        timeSinceLastChange += 1000000 / sampleRate;
        writeSoftState();
    }
}
...
int main()
{
    cout << "Soft factor (x > 0; x <= 1 is soft; x >= 1 is sharp): ";
    cin >> softFactor;
    setup();
    loop();
    ww.write();
